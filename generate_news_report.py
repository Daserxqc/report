...
def process_industry_trends(llm_processor, topic, trend_news):
    ...
    trend_prompt = f"""
    请基于以下{topic}行业的最新趋势相关新闻，分析并总结行业整体趋势和发展方向，特别关注不同来源对行业趋势的不同解读和预测。

    {all_news_text}
    
    请提供详细的行业趋势分析，内容需要包括：
    1. {topic}行业的整体发展趋势和主要特征，以及各方对此的不同解读
    2. 市场规模、增长率和主要驱动因素的多角度分析
    3. 技术发展路线和创新焦点的不同观点对比
    4. 值得关注的新技术、新产品或新模式，以及各方对其前景的不同预测
    5. 行业面临的挑战、机遇和潜在风险的多角度分析
    6. 区域发展差异和国际竞争格局的不同解读
    7. 产业链上下游发展情况的各方观点对比
    8. 对未来3-5年的预测和展望，包括不同来源的预测差异
...
你特别注重对不同来源的趋势预测进行对比分析，能够客观地呈现各方观点。
