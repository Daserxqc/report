...
"""
            ...
            # 检查结果是否为协程对象
            if asyncio.iscoroutine(enhanced_content):
                print(f"⚠️ 警告: LLM API返回协程对象，尝试等待结果")
                try:
                    enhanced_content = asyncio.run(enhanced_content)
                except Exception as e:
                    print(f"⚠️ 错误: 无法等待协程结果: {e}")
                    return existing_content
            ...
            if not isinstance(enhanced_content, str):
                enhanced_content = str(enhanced_content) if enhanced_content else existing_content
